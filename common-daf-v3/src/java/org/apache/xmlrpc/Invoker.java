...
public class Invoker implements XmlRpcHandler
{
    ...
    public Invoker(Object target)
    {
        ...
        if (RPC.isDebug()) {
            logger.debug("Invoker.init - " + targetClass);
        }
    }
    ...
    public Object execute(String methodName, Vector params) throws Exception
    {
        ...
        {
            if (RPC.isDebug()) {
                logger.debug("Invoker.execute - searching for method: " + methodName + " in class: " + targetClass.getName());
            }
            for (int i = 0; i < argClasses.length; i++)
            {
                if (RPC.isDebug()) {
                    logger.debug("Invoker.execute - parameter " + i + ": " + argValues[i] + " (" + argClasses[i] + ')');
                }
            }
        }
