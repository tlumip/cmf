...
public class OMXMatrixReader extends MatrixReader {
	...
	private String lookupName = null; //uses first lookup as zone names if found
    ...
    */
    private Matrix readData(String name) {
    	...
    	try {
            ...
            //add external zone numbers using first lookup if found
            if (omxFile.getLookupNames().size() > 0 ) {
            	String firstLookupName = omxFile.getLookupNames().iterator().next();
            	OmxLookup.OmxIntLookup omxZoneNums = (OmxLookup.OmxIntLookup)omxFile.getLookup(firstLookupName);
